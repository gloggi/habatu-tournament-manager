...
export const gamesGenerator = (
  ...
  for (let category of categories) {
    teamsByCategory[category._id.toString()] = teams.filter(
      (team) => team.category == category._id.toString()
    );
  }
  ...
  for (let category in teamsByCategory) {
    ...
    for (let i = 0; i < roundTeams.length; i++) {
      ...
      for (let j = 1; j < cTeams.length / 2; j++) {
        ...
        const teamB =
          roundTeams[(i - j + roundTeams.length) % roundTeams.length];
        gamesByCategory[category].push({
  ...
  const games: IGame[] = flattenDeep<Array<IGame>>(
    zip<IGame>(...tempGames) as IGame[][]
  ).filter((g) => g);
...
export const generateTimeslots = (
  ...
  for (let i = 0; i < nslots; i++) {
    timeSlots.push({
      ...
      endTime: addMinutes(
        startTime,
        parseInt(option.gameDuration.split(":")[1])
      ),
    });
