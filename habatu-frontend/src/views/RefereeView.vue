<template>
    <div class="justify-stretch flex h-screen flex-col items-center">
        ...
        <div class="flex h-full w-full flex-col space-y-2 rounded-md bg-white p-3 drop-shadow-lg md:w-1/3 text-center">
            <template v-for="game in myGames" :key="game._id">
                {{ formatTimeslot(game.timeslot) }}
                <router-link :to="{name:'gameRefereeingView', params:{id: game._id}}">
...
export default {
    ...
    methods: {
        formatTimeslot(timeslot) {
            return `${format(new Date(timeslot.startTime), "HH:mm")} - ${format(new Date(timeslot.endTime), "HH:mm")}`
