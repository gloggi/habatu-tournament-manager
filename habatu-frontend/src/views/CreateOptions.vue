<template>
  ...
  <div class="w-full h-full  rounded-lg border flex flex-row text-left">
    ...
    <form @submit.prevent="setOptions" :key="formKey" class="w-full flex flex-col space-y-3 border-r p-5 ">
        <h1 class="text-2xl font-bold pb-3">Einstellungen</h1>
        <JsonForm v-if="items" @changeForm="handleMainFormChange" :form="form" :values="items" />
    ...
    <div class="w-full p-3 mb-10">
        <div v-if="timePreview">
            ...
            <h1 class="text-2xl font-bold">Anzahl Spiele: <span class="font-normal">{{timePreview.amountOfGames}}</span></h1>
            <h1 class="text-2xl font-bold">Letztes reguläres Spiel endet um: <span class="font-normal">{{formatDate(new Date(timePreview.lastGame), "HH:mm")}}</span></h1>
        </div>
    </div>
...
export default {
    ...
    data(){
        return {
            ...
            form: [
                [
                    { label: "Startzeit", model: "startTime", component: "TextField", type: "time" }
                ],
                [
                    { label: "Spielzeit", model: "gameDuration", component: "TextField", type: "time" }
                ],
                [
                    { label: "Pausenzeit", model: "breakDuration", component: "TextField", type: "time" }
                ]
            ]
        }
    },
