<template>
    <div v-if="game" class="min-h-screen h-full w-screen text-white  -m-2" :style="`${!gameOver?'background-color: rgb(34 197 94)':'background-color: rgb(239 68 68)'}`">
        <div class="flex flex-col space-y-2">
            <div class="flex flex-col space-x-1 p-3 items-center">
                <div class="font-medium">Timer</div>
                <div class="font-medium text-7xl">{{timer}}</div>
            </div>
            <div class="flex flex-col justify-around text-center">
                <div class="flex flex-row">
                    <div class="text-2xl w-1/2 font-medium">
                        {{ game.teamA.name }}
                    </div>
                    <div class="text-2xl w-1/2  font-medium">
                        {{ game.teamB.name }}
                ...
                <div class="flex flex-row">
                    <div class="text-2xl w-1/2 font-light mb-2">
                ...
                <div class="flex flex-row">
                    <div class="text-9xl mb-4 w-1/2 font-medium ">
                        {{ game.pointsTeamA }}
                    </div>
                    <div class="text-9xl w-1/2 font-medium  mb-4">
                        {{ game.pointsTeamB }}
                    </div>
            ...
            <div class="p-5 flex flex-col space-y-2">
                <BasicButton class="bg-green-700 hover:bg-green-700" v-if="!started" @click="startTimer">Start</BasicButton>
                <BasicButton class="bg-orange-700 hover:bg-orange-700" v-if="!started" @click="clearTimer">Nomal vo vorne</BasicButton>
                <BasicButton class="bg-red-500 hover:bg-red-500" v-if="started" @click="stopTimer">Stop</BasicButton>
            </div>
            <div class="p-5 flex flex-col space-y-2">
                <BasicButton class="bg-black hover:bg-black" @click="reportGame">Spielstand abschicke</BasicButton>
...
export default {
    ...
    methods: {
        ...
        modifyPoints(team, value) {
            ...
            localStorage.setItem(`${this.game._id}:pointsTeamA`, this.game.pointsTeamA)
            localStorage.setItem(`${this.game._id}:pointsTeamB`, this.game.pointsTeamB)
        ...
        async reportGame(){
            try {
               ...
               this.$router.push({name:"menu"})
                ...
                this.$store.commit("notifications/showNotification", {message: "Kei Verbindig!", type: false})
            }
    ...
    async created() {
        ...
        this.gameDuration = parseInt(this.$store.state.options.options.gameDuration.split(":")[1])*60*1000
