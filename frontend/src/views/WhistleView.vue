...
<template>
  <div class="p-5 h-screen flex justify-center items-center">
    <div class="flex flex-col h-full w-full justify-between">
      <div
        class="w-full flex justify-around text-2xl font-semibold break-all space-x-2"
      >
        <p>{{ game?.teamA.name }}</p>
        <p>vs.</p>
        <p>{{ game?.teamB.name }}</p>
      ...
      <div
        class="w-full flex justify-around text-2xl font-semibold break-all space-x-2"
      >
        <p>{{ game?.teamA.section?.name }}</p>
        <p></p>
        <p>{{ game?.teamB.section?.name }}</p>
      </div>
      <div>
        <div class="flex justify-center font-semibold text-4xl">Zeit</div>
        <div class="flex justify-center font-semibold text-7xl">
          {{ format(timer, "mm:ss") }}
        </div>
      </div>
      <div>
        ...
        <div class="flex justify-around font-semibold">
          <Button
            @click="changePoints(1, 'A')"
            class="h-full aspect-square"
            variant="ghost"
            ><PlusIcon class="size-24"
          /></Button>
          <Button
            @click="changePoints(1, 'B')"
            class="h-full aspect-square"
            variant="ghost"
            ><PlusIcon class="size-24"
          /></Button>
